...
\caption{Het aantal \textit{false positives} ten opzichte van het aantal documenten behorend tot die partij (\textit{false negatives} en \textit{true positives}). Dit is op basis van 50 classificaties met verschillende test en train set. De pearson correlatie is 0.77 en de p-waarde \num{5.40e-101}.}
...
\subsubsection{DV4: Links/rechts}
Als de classificatie afhankelijk is van links/rechts positie, dan is het te verwachten dat, net als bij deelvraag 3, de misclassificaties minus de verwachte waarde groter zijn als twee partijen dichterbij elkaar staan op de links/rechts as. Daarvoor zal wederom formule \ref{eq:expected} gebruikt worden als verwachte waarde en formule \ref{eq:error} als error.\par
